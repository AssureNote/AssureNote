...
export class AssuranceCase {
	constructor(public hashKey:string, public userKey:number, public data:string, public meta_data?:string) {
	}
...
export class AssuranceCaseDAO extends model.DAO {
	...
	insert(userKey: string, data: string, meta_data: string, callback: (err:any, hashKey: string)=>void): void {
		...
		async.waterfall([
			(next) => {
				this.con.query('INSERT INTO `assurance_case` (`hash_key`, `data`, `meta_data`, `user_key`) VALUES (?, ?, ?, ?)', [hashKey, data, meta_data, userKey], (err, result) => next(err, result));
			}],
			(err:any, result:any) => {
				if (err) {
					callback(err, null);
