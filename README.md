...
### メニューバー
![MenuBar](/sample/images/MenuBar.png)

メニューバーはノード上で右クリックを押すことで表示され、
ノードの追加や削除・編集のための機能を呼び出すことができます。
上の画像を用いて、メニューバーの機能の説明を左から順に行っていきます。

#### 編集（単一ノード）
鉛筆の記号で表される項目です。
メニューバーが表示されたノードの編集を行うためのエディタが開きます。
エディタは、Escキーを押すか、エディタの外をクリックすることで閉じます。

#### 編集（複数ノード）
文章の記号で表される項目です。
メニューバーが表示されたノードの部分木を一括して編集するためのエディタが開きます。
エディタの記法は、以降で説明されるWGSNを用います。

#### ノードの追加
ノードの追加を行います。
それぞれの項目は、GSNの図形の形をしています。
追加できるノードはノードの種類毎に異なる点に注意して下さい。
例えば、Goalに対してGoalを追加することはできません。
したがって、上記の画像でもGoalの追加メニューは表示されていません。
